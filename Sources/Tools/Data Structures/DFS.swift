...
//
//
//  Created by Tyler Dailey on 12/25/23.
//

import Collections
import Foundation

public protocol DFSGrid {
    func reachableNeighborsAt(position: Point2D) -> [Point2D]
}

/// Various solvers using the DFS strategy
public enum DFS {}

// MARK: - Shortest path in grid from A to B

public extension DFS {
    struct LongestPathInGridResult {
    ...
    private struct LongestPathInGridQueueNode {
    ...
    static func longestPathInGrid(_ grid: some DFSGrid, from pointA: Point2D, to pointB: Point2D) -> LongestPathInGridResult? {
        var solutionQueue: Deque<LongestPathInGridQueueNode> = [
            .init(position: pointA, visitedPoints: [pointA], steps: 0),
        ]

        var bestSolution: LongestPathInGridQueueNode?

        var visitedPoints: Set<Point2D> = [pointA]

        while let solution = solutionQueue.popLast() {
